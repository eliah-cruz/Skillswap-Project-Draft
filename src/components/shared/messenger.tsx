  ...
  return (
    <>
      ...
      <div className={`fixed flex flex-col bg-white shadow-2xl transition-all duration-500 ease-in-out ${containerClasses} ${visibilityClasses}`}>
        {/* Header */}
        <div className={`p-6 border-b-2 border-slate-50 flex justify-between items-center bg-white sticky top-0 z-20 ${isMaximized ? 'rounded-t-[3rem]' : ''}`}>
          <div className="flex items-center gap-4">
            {partner ? (
              <>
                <div className="relative">
                  <div className={`w-14 h-14 rounded-2xl text-white flex items-center justify-center font-black text-xl shadow-lg overflow-hidden border-2 border-white ${partner.rating < 4.0 ? 'bg-red-600' : 'bg-indigo-600'}`}>
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-6 bg-slate-900/80 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-[3rem] w-full max-w-md p-10 shadow-2xl">
            ...
            <div className="flex gap-4">
              ...
              <button type="button" onClick={() => { actions.deleteConversation(partner.id); setShowDeleteModal(false); }} className="flex-1 bg-red-500 text-white font-black py-4 rounded-2xl text-[11px] uppercase tracking-tight hover:bg-red-600 shadow-xl transition cursor-pointer flex items-center justify-center">Confirm Delete</button>
            </div>
          </div>
        </div>
      )}
