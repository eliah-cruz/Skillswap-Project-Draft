      ...
      <div className="sticky top-24 z-20 space-y-4">
        <div className="flex flex-col md:flex-row gap-4 max-w-6xl mx-auto">
          <div className="relative group flex-1">
            ...
            />
            {state.searchQuery && (
              <button type="button" onClick={() => setters.setSearchQuery("")} className="absolute inset-y-0 right-6 flex items-center text-slate-300 hover:text-red-500 cursor-pointer transition-colors">
                <X size={24} />
              </button>
            )}
          </div>
          ...
          <div className="bg-white p-2 rounded-[2.5rem] border-4 border-slate-100 shadow-xl flex items-center gap-1 shrink-0 overflow-x-auto no-scrollbar">
            ...
            {["Recommended", "Top Rated", "Newest"].map((sort) => (
                <button type="button" key={sort} onClick={() => setters.setSortBy(sort)} className={`whitespace-nowrap px-6 py-3 rounded-full text-[11px] font-black uppercase tracking-tight transition-all cursor-pointer ${state.sortBy === sort ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}>
                    {sort}
                </button>
            ))}
          </div>
        ...
        <div className="bg-white p-3 rounded-[2.5rem] border-2 border-slate-200 flex flex-col xl:flex-row justify-between shadow-2xl shadow-indigo-100/40 gap-3 overflow-hidden">
            ...
            <div className="flex items-center justify-between xl:justify-end gap-4 xl:border-l-2 border-slate-100 xl:pl-6 px-4 xl:pr-6 shrink-0 pt-2 xl:pt-0 border-t-2 xl:border-t-0 border-slate-100">
                ...
                <button type="button" onClick={() => setters.setOnlineOnly(!state.onlineOnly)} className={`cursor-pointer w-12 h-6 rounded-full transition-all relative border-2 ${state.onlineOnly ? 'bg-emerald-500 border-emerald-600' : 'bg-slate-300 border-slate-400'}`}>
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        ...
        <div className="flex-1">
          {!state.hasSkillsConfigured ? (
            <div className="bg-slate-50 rounded-[4rem] border-4 border-dashed border-slate-200 p-16 md:p-24 text-center max-w-4xl mx-auto shadow-inner animate-in fade-in duration-300">
              <div className="w-20 h-20 bg-indigo-50 text-indigo-600 rounded-[2rem] flex items-center justify-center mx-auto mb-8 shadow-xl animate-pulse">
                <Sparkles size={32} />
              </div>
              ...
              <p className="text-slate-500 font-bold text-sm max-w-md mx-auto leading-relaxed">
                To unlock discovery, add at least <span className="text-indigo-600">one teachable skill</span> and <span className="text-emerald-600">one desired skill</span> inside the <span className="font-extrabold text-slate-800">"My Skill Profile"</span> sidebar dashboard on the right.
              </p>
              <div className="mt-8 flex justify-center gap-4">
                <button type="button" onClick={() => { setters.setAddingSkillType('teaching'); setters.setShowDirectory(true); }} className="bg-indigo-600 text-white px-6 py-3 rounded-2xl text-[11px] font-black uppercase tracking-wider hover:bg-slate-900 transition-all cursor-pointer shadow-lg shadow-indigo-100">
                  + Add Teachable Skill
                </button>
                <button type="button" onClick={() => { setters.setAddingSkillType('learning'); setters.setShowDirectory(true); }} className="bg-emerald-600 text-white px-6 py-3 rounded-2xl text-[11px] font-black uppercase tracking-wider hover:bg-slate-900 transition-all cursor-pointer shadow-lg shadow-emerald-100">
                  + Add Desired Skill
                </button>
              </div>
            </div>
            ...
            <div>
                    ...
                    <div className="p-8 flex-1 flex flex-col relative text-left">
                      ...
                      <div className="relative z-10 flex-1">
                        ...
                        <div className="flex gap-5 mb-2 pr-20">
                            ...
                            <div className="min-w-0 flex-1 pt-1 text-left">
                                ...
                                <div className="flex flex-wrap gap-x-2 gap-y-2.5">
                                    ...
                                    <div className="text-[10px] font-black uppercase text-slate-500 tracking-wider px-3 py-1.5 bg-slate-50 rounded-xl border border-slate-200">
                                      {m.availability}
                                    </div>

                                    {m.experienceLevel && (
                                      <div className="text-[10px] font-black uppercase text-indigo-700 tracking-wider px-3 py-1.5 bg-indigo-50 rounded-xl border border-indigo-100 flex items-center gap-1.5 shadow-sm">
                                        <GraduationCap size={12} className="text-indigo-500" /> {m.experienceLevel}
                                      </div>
                                    )}

                                    <div className="text-[10px] font-black uppercase text-amber-700 tracking-wider px-3 py-1.5 bg-amber-50 rounded-xl border border-amber-200 flex items-center gap-1.5 shadow-sm">
                                      <Coins size={12} className="fill-amber-500 text-amber-500" /> {m.hoursBalance ?? 3} HR BAL
                                    </div>

                                    {m.location && (
                                      <div className="text-[10px] font-black uppercase text-emerald-700 tracking-wider px-3 py-1.5 bg-emerald-50 rounded-xl border border-emerald-100 flex items-center gap-1.5 shadow-sm">
                                        <MapPin size={12} className="text-emerald-500" /> {m.location}
                                      </div>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className="space-y-4 mb-4">
                            <div className={`p-4 rounded-[1.5rem] border text-left relative overflow-hidden transition-colors ${isLowRated ? 'bg-red-50/30 border-red-100' : m.isMutualMatch ? 'bg-indigo-50/50 border-indigo-200' : 'bg-slate-50 border-slate-100 group-hover:bg-indigo-50/30'}`}>
                                <p className={`text-[10px] font-black uppercase tracking-widest mb-1.5 flex items-center gap-2 relative z-10 ${isLowRated ? 'text-red-600' : 'text-indigo-600'}`}><BookOpen size={14} /> Will Teach You</p>
                                <p className="text-[15px] font-bold text-slate-900 relative z-10">{m.teaching}</p>
                            </div>
                            <div className={`p-4 rounded-[1.5rem] border text-left relative overflow-hidden transition-colors ${isLowRated ? 'bg-red-50/30 border-red-100' : m.isMutualMatch ? 'bg-emerald-50/70 border-emerald-300' : 'bg-emerald-50/50 border-emerald-100 group-hover:bg-emerald-100/50'}`}>
                                <p className={`text-[10px] font-black uppercase tracking-widest mb-1.5 flex items-center gap-2 relative z-10 ${isLowRated ? 'text-red-600' : 'text-emerald-600'}`}><Sparkles size={14} /> Wants to Learn</p>
                                <p className="text-[15px] font-bold text-emerald-950 relative z-10">{m.needs}</p>
                            </div>
                        </div>
                          ...
                          <div className="mt-4 mb-4 p-4 bg-teal-50/70 border border-teal-200 rounded-2xl text-left animate-in slide-in-from-top-2 duration-300">
                            ...
                            <div className="flex items-center justify-between text-center gap-1">
                              <div className="flex flex-col items-center">
                                ...
                                <span className="text-[8px] font-black text-slate-400 uppercase mt-1">Teaches</span>
                              </div>

                              <ArrowRight size={14} className="text-teal-400" />

                              <div className="flex flex-col items-center">
                                <div className="w-8 h-8 rounded-xl bg-teal-600 text-white flex items-center justify-center text-[10px] font-black uppercase">
                                  {m.name.substring(0, 2)}
                                </div>
                                <span className="text-[8px] font-black text-teal-600 uppercase mt-1">{m.name.split(' ')[0]}</span>
                              </div>
                              ...
                              <div className="flex flex-col items-center">
                                ...
                                <span className="text-[8px] font-black text-slate-400 uppercase mt-1">Learns</span>
                              </div>
                            </div>
                          </div>
                        )}
                      ...
                      <div className="flex gap-3 mt-auto relative z-10">
                        <button type="button" onClick={() => goToChat(m)} className={`flex-1 cursor-pointer text-white py-4 rounded-2xl font-black text-[11px] uppercase tracking-widest active:scale-95 transition-all flex items-center justify-center gap-2 shadow-xl ${isLowRated ? 'bg-red-600 hover:bg-slate-900' : 'bg-slate-900 hover:bg-indigo-600'}`}>
                        ...
                        <button 
                          ...
                          className="cursor-pointer bg-red-50 text-red-400 hover:text-white hover:bg-red-500 p-4 rounded-2xl transition-all border border-red-100 hover:border-red-500 shadow-sm flex items-center justify-center shrink-0"
                          title="Report User"
                        >
                          <Flag size={18} />
                        </button>
              ...
              {totalPages > 1 && (
                <div className="mt-12 flex justify-center items-center gap-2 animate-in fade-in duration-500 pb-8">
                  <button 
                    type="button"
                    disabled={currentPage === 1}
                    onClick={() => handlePageChange(currentPage - 1)}
                    className="cursor-pointer px-4 py-2 border-2 border-slate-200 rounded-xl text-xs font-black uppercase text-slate-500 hover:border-indigo-600 hover:text-indigo-600 transition-all disabled:opacity-30 disabled:pointer-events-none"
                  >
                    Prev
                  </button>
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => (
                    <button
                      ...
                      className={`cursor-pointer w-10 h-10 rounded-xl text-xs font-black border-2 transition-all ${
                        currentPage === pageNum
            ...
            <div className="bg-slate-50 rounded-[4rem] border-4 border-dashed border-slate-200 p-24 text-center">
              ...
              <button type="button" onClick={() => { setters.setActiveCategoryFilter("All"); setters.setSearchQuery(""); setters.setSortBy("Recommended"); }} className="mt-6 bg-indigo-600 text-white px-8 py-3 rounded-full font-black text-xs uppercase tracking-widest cursor-pointer shadow-lg hover:bg-indigo-700 transition-all">Clear Everything</button>
            </div>
          )}
        </div>
        ...
        <aside className="lg:w-96 space-y-8">
                      ...
                      <div className="flex flex-wrap gap-2 mb-3">
                          {state.myNeeds.length > 0 ? (
                              state.myNeeds.map((s: string) => (
                                  <div key={s} className="flex items-center gap-2 px-3 py-1.5 bg-emerald-500/15 text-emerald-300 border border-indigo-500/20 rounded-lg text-[11px] font-black uppercase tracking-tight transition-all hover:border-emerald-400/50">{s} <button type="button" onClick={() => actions.removeNeed(s)} className="text-emerald-400 hover:text-emerald-100 cursor-pointer transition-colors">✕</button></div>
                              ))
                          ) : (
            ...
            <div className="bg-white p-8 rounded-[3rem] border-2 border-indigo-50 shadow-sm text-left">
                        ...
                        <div key={user.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              {isReported && (
                                <span className="text-[8px] text-red-500 font-extrabold uppercase tracking-widest block mt-0.5">Reported (Pending Admin)</span>
                              )}
                            </div>
                          </div>
